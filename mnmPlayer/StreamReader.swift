...
class StreamReader  {
    ...
    init?(url: NSURL, delimiter: String = "\n", encoding : UInt = NSUTF8StringEncoding, chunkSize : Int = 4096) {
        ...
        do {
            self.fileHandle = try NSFileHandle.init(forReadingFromURL: url)
            self.delimData = delimiter.dataUsingEncoding(encoding)
            self.buffer = NSMutableData(capacity: chunkSize)
        } catch (let err) {
            print("Error: StreamReader could not open: \(url.absoluteString) for reading:\(err)")
